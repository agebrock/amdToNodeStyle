...
exports.parse = function (raw) {
    ...
    if (defines.length > 1) {
        console.log('Warn:Multiple "define" calls in a single file are not supported. Found "' + defines.length + '"');
        return raw;
    ...
    if (!factory) {
        console.log('Warn: could not parse AMD style');
        return raw;
    }
...
function getUseStrict(factory) {
    if (factory && factory.body && factory.body.body) {
